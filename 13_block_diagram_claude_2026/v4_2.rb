...
module SyntaxHighlighter
  # Langages connus de GtkSourceView (ids standards)
  KNOWN_LANGUAGES = %w[
    ruby python c cpp java javascript typescript rust go
    sh bash perl lua r matlab octave sql html css xml
    yaml json makefile cmake diff latex
  ].freeze

  @json_rules  = []
  @custom_lang = nil   # GtkSource::Language construit depuis syntax.json
  @lang_manager = nil
  ...
  def self.load_from_file(path = nil)
    ...
    candidates.each do |f|
      next unless f && File.exist?(f)
      data = JSON.parse(File.read(f))
      @json_rules = data['rules'] if data['rules'].is_a?(Array)
      return f
    end
    nil
  end

  def self.json_rules = @json_rules

  # Retourne un GtkSource::Language pour l'identifiant donné.
  # Si non reconnu, retourne nil (le caller utilisera le fallback JSON).
  def self.language_for_id(lang_id)
    ...
    # ruby-gnome expose get_language ou [] selon la version de la gem
  ...
  def self.dark_scheme
    ...
    # L'API ruby-gnome expose get_scheme (ou l'opérateur []) selon la version
  ...
  def self.known_languages = KNOWN_LANGUAGES
end
...
class CodeEditorDialog < Gtk::Dialog
  ...
  def build_content
    ...
    @lang_combo.append_text('(custom syntax.json)')

    current = @block.language.to_s.strip
    idx = if current.empty? || current == '(aucun)'
            0
          elsif current == '(custom syntax.json)'
            SyntaxHighlighter.known_languages.size + 1
          else
  ...
  def apply_language(lang_id)
    ...
    when '(custom syntax.json)'
      @source_buffer.language = build_custom_language
    else
      lang = SyntaxHighlighter.language_for_id(lang_id)
      @source_buffer.language = lang || build_custom_language
    end
  ...
  # Construit un GtkSource::Language à partir de syntax.json en écrivant
  # un fichier .lang XML temporaire et en l'injectant dans un LanguageManager.
  def build_custom_language
    rules = SyntaxHighlighter.json_rules
    return nil if rules.empty?

    xml = build_lang_xml(rules)
    dir = File.join(Dir.tmpdir, 'bde_lang')
    FileUtils.mkdir_p(dir)
    lang_file = File.join(dir, 'bde_custom.lang')
    File.write(lang_file, xml)

    mgr = GtkSource::LanguageManager.new
    mgr.search_path = [dir] + GtkSource::LanguageManager.default.search_path
    if mgr.respond_to?(:get_language)
      mgr.get_language('bde_custom')
    else
      mgr['bde_custom']
    end
  rescue => e
    warn "build_custom_language: #{e.message}"
    nil
  end

  def build_lang_xml(rules)
    items = rules.map do |rule|
      color   = rule['color'] || '#cccccc'
      bold    = rule['bold']   ? ' bold="true"'   : ''
      italic  = rule['italic'] ? ' italic="true"' : ''
      style_id = "bde_custom:#{rule['name']}"

      contexts = []
      if rule['patterns']
        words = rule['patterns'].map { |w| Regexp.escape(w) }.join('|')
        contexts << <<~CTX
          <context id="#{rule['name']}" style-ref="#{style_id}">
            <match>\\b(#{words})\\b</match>
          </context>
        CTX
      elsif rule['regex']
        safe = rule['regex'].gsub('&', '&amp;').gsub('<', '&lt;').gsub('>', '&gt;')
        contexts << <<~CTX
          <context id="#{rule['name']}" style-ref="#{style_id}">
            <match>#{safe}</match>
          </context>
      ...
      { style_id: style_id, color: color, bold: bold, italic: italic, contexts: contexts }
    end

    styles = items.map do |it|
      "<style id=\"#{it[:style_id]}\" foreground=\"#{it[:color]}\"#{it[:bold]}#{it[:italic]}/>"
    end.join("\n    ")

    contexts_xml = items.flat_map { |it| it[:contexts] }.join
    ctx_refs = items.map { |it| "<context ref=\"#{it[:style_id].split(':').last}\"/>" }.join("\n        ")

    <<~XML
      <?xml version="1.0" encoding="UTF-8"?>
      <language id="bde_custom" name="BDE Custom" version="2.0" _section="Source">
        <metadata>
          <property name="mimetypes">text/x-bde</property>
          <property name="globs">*.bde</property>
        </metadata>
        <styles>
          #{styles}
        </styles>
...
class MainWindow < Gtk::ApplicationWindow
  def initialize(app, filepath = nil)
    ...
    SyntaxHighlighter.load_from_file
