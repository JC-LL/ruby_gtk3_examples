...
class SnippetManager
  ...
  def build_ui
    ...
    # Create main horizontal box - Celle-ci s'expand correctement
    main_box = Gtk::Box.new(:horizontal, 10)
    main_box.set_margin_top(10)
    main_box.set_margin_bottom(10)
    main_box.set_margin_start(10)
    main_box.set_margin_end(10)
    main_box.set_hexpand(true)  # Important pour le redimensionnement horizontal
    main_box.set_vexpand(true)  # Important pour le redimensionnement vertical
    @window.add(main_box)

    # Left panel for snippets - Fixe en largeur mais expand en hauteur
    left_panel = Gtk::Box.new(:vertical, 5)
    left_panel.set_size_request(250, -1) # Largeur fixe, hauteur flexible
    left_panel.set_vexpand(true) # S'expand verticalement
    ...
    # Snippets list with scroll - Doit s'expand
    snippets_frame = Gtk::Frame.new
    snippets_frame.set_shadow_type(:etched_in)
    snippets_frame.set_vexpand(true) # Très important
    ...
    scrolled_snippets.set_vexpand(true) # S'expand verticalement
    scrolled_snippets.set_min_content_height(200) # Hauteur minimale réduite
    ...
    # Control buttons - En bas, ne s'expand pas
    controls_box = Gtk::Box.new(:vertical, 5)
    controls_box.set_vexpand(false) # Ne s'expand pas
    ...
    # Right panel for source editor - Doit s'expand dans toutes les directions
    right_panel = Gtk::Box.new(:vertical, 5)
    right_panel.set_hexpand(true)  # S'expand horizontalement
    right_panel.set_vexpand(true)  # S'expand verticalement
    ...
    # Scrollable source area - Doit s'expand complètement
    scrolled_source = Gtk::ScrolledWindow.new
    scrolled_source.set_policy(:automatic, :automatic)
    scrolled_source.set_hexpand(true)  # S'expand horizontalement
    scrolled_source.set_vexpand(true)  # S'expand verticalement
    scrolled_source.add(@source_view)
    right_panel.pack_start(scrolled_source, expand: true, fill: true, padding: 0)

    # Status bar - Ne s'expand pas
  ...
  def create_source_view
    ...
    # Create source view avec le buffer
    ...
    @source_view.wrap_mode = :word_char
    @source_view.set_hexpand(true)  # S'expand horizontalement
    @source_view.set_vexpand(true)  # S'expand verticalement
