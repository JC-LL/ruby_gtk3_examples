...
class SnippetManager
  ...
  def build_ui
    ...
    snippets_label = Gtk::Label.new
    snippets_label.set_markup("<b>Snippets (#{@snippets.size})</b>")
    snippets_header.pack_start(snippets_label, expand: true, fill: true, padding: 0)
    ...
    @snippets_listbox.signal_connect('row-activated') do |listbox, row|
      if row && row.snippet_name
        load_snippet_to_editor(row.snippet_name)
      end
    end
  ...
  def update_snippets_list
    ...
    @filtered_snippets.keys.sort.each do |snippet_name|
      ...
      row.add(box)
      row.snippet_name = snippet_name  # Store snippet name for retrieval
  ...
  def load_snippet_to_editor(snippet_name)
    if @snippets[snippet_name]
      ...
      @snippets_listbox.children.each do |row|
        if row.snippet_name == snippet_name
