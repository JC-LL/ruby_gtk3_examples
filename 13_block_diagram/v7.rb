...
class Port
  attr_accessor :name, :type, :direction, :x, :y, :width, :height, :connections, :block, :canvas_port

  def initialize(name = "port", type = "any", direction = :input, block = nil, canvas_port = false)
    @name = name
    @type = type
    @direction = direction
    @x = 0
    ...
    @connections = []
    @block = block
    @canvas_port = canvas_port  # true si c'est un port libre dans le canvas
  end
...
class Block
  ...
  def add_input_port(name = "in#{@input_ports.size + 1}", type = "any")
    port = Port.new(name, type, :input, self, false)
    @input_ports << port
    update_ports_position
    port
  end
  ...
  def add_output_port(name = "out#{@output_ports.size + 1}", type = "any")
    port = Port.new(name, type, :output, self, false)
    @output_ports << port
    update_ports_position
    port
  end
...
class Canvas < Gtk::DrawingArea
  attr_accessor :blocks, :current_block, :drag_start, :drag_offset, :selected_block, :resizing_handle,
                :connections, :temp_connection, :selected_port, :context_menu_block, :canvas_ports,
                :drag_port, :port_drag_offset
  ...
  def initialize
    ...
    @canvas_ports = []  # Ports libres dans le canvas
    @drag_port = nil
    @port_drag_offset = nil
  ...
  def setup_context_menu
    ...
    # Menu pour les ports canvas
    delete_port_item = Gtk::MenuItem.new(label: "Supprimer le port")
    delete_port_item.signal_connect("activate") { delete_selected_port }
    @context_menu.append(delete_port_item)
  ...
  def key_press(widget, event)
    ...
    # Ctrl+Maj+I pour ajouter un port d'entrée libre dans le canvas
    if keyval == Gdk::Keyval::KEY_i &&
       (state & Gdk::ModifierType::CONTROL_MASK != 0) &&
       (state & Gdk::ModifierType::SHIFT_MASK != 0)
      add_canvas_port(:input)
      return true
    end

    # Ctrl+Maj+O pour ajouter un port de sortie libre dans le canvas
    if keyval == Gdk::Keyval::KEY_o &&
       (state & Gdk::ModifierType::CONTROL_MASK != 0) &&
       (state & Gdk::ModifierType::SHIFT_MASK != 0)
      add_canvas_port(:output)
      return true
    end
  ...
  def add_canvas_port(direction)
    dialog = PortEditor.new(self, nil, direction)
    response = dialog.run
    if response == Gtk::ResponseType::OK
      # Positionner le port libre au centre du canvas
      allocation = self.allocation
      x = allocation.width / 2 - 6
      y = allocation.height / 2 - 6

      port = Port.new(dialog.name, dialog.type, direction, nil, true)
      port.x = x
      port.y = y
      @canvas_ports << port
      puts "Port #{direction == :input ? 'd\'entrée' : 'de sortie'} libre ajouté au canvas: #{port.name}"
      queue_draw
    end
    dialog.destroy
  end
  ...
  def delete_selected_port
    return unless @selected_port

    # Supprimer les connexions associées
    remove_connections_for_port(@selected_port)

    # Supprimer le port de son conteneur (bloc ou canvas)
    if @selected_port.canvas_port
      @canvas_ports.delete(@selected_port)
      puts "Port libre #{@selected_port.name} supprimé"
    elsif @selected_port.block
      @selected_port.block.remove_port(@selected_port)
      puts "Port #{@selected_port.name} supprimé du bloc #{@selected_port.block.id}"
    end
  ...
  def draw(widget, cr)
    ...
    # Dessiner les ports libres du canvas
    draw_canvas_ports(cr)
  ...
  def draw_shortcuts_info(cr)
    ...
    info = "Ctrl+I/O: ports bloc | Ctrl+Maj+I/O: ports libres | Clic-droit: supprimer"
    cr.move_to(10, 20)
    cr.show_text(info)
  end

  def draw_canvas_ports(cr)
    @canvas_ports.each do |port|
      draw_canvas_port_shape(cr, port)
    end
  end

  def draw_canvas_port_shape(cr, port)
    center_x = port.center_x
    center_y = port.center_y
    radius = port.width / 2

    # Dessiner un cercle plus grand pour les ports canvas
    big_radius = radius * 1.5

    if port == @selected_port
      cr.set_source_rgb(1.0, 0.8, 0.0) # Jaune pour sélectionné
    else
      case port.direction
      when :input
        cr.set_source_rgb(0.2, 0.8, 0.2) # Vert pour entrées
      when :output
        cr.set_source_rgb(0.8, 0.2, 0.2) # Rouge pour sorties
      end
    end

    # Cercle extérieur plus grand
    cr.arc(center_x, center_y, big_radius, 0, 2 * Math::PI)
    cr.fill

    # Cercle intérieur
    cr.set_source_rgb(0.1, 0.1, 0.1) # Noir pour l'intérieur
    ...
    if port.direction == :input
      cr.move_to(port.x - extents.width - 8, port.y + big_radius + 12)
    else
      cr.move_to(port.x + big_radius + 5, port.y + big_radius + 12)
    end
  ...
  def handle_right_click(x, y)
    # Vérifier si on clique sur un port (canvas ou bloc)
    clicked_port = find_port_at(x, y)
    if clicked_port
      @selected_port = clicked_port
  ...
  def find_port_at(x, y)
    # Chercher d'abord dans les ports canvas (priorité pour le drag)
    canvas_port = @canvas_ports.find { |port| port.contains?(x, y) }
    return canvas_port if canvas_port

    # Chercher dans les ports des blocs
    @blocks.each do |block|
  ...
  def handle_port_click(port, x, y)
    ...
    # Si c'est un port canvas, permettre le drag
    if port.canvas_port
      @drag_port = port
      @port_drag_offset = { x: x - port.x, y: y - port.y }
      return
    end
  ...
  def handle_double_click(x, y)
    # Double-clic sur un port pour l'éditer
    clicked_port = find_port_at(x, y)
    if clicked_port
      dialog = PortEditor.new(self, clicked_port, clicked_port.direction)
      response = dialog.run
      if response == Gtk::ResponseType::OK
        clicked_port.name = dialog.name
        clicked_port.type = dialog.type
        puts "Port #{clicked_port.name} mis à jour"
      end
      dialog.destroy
      return
    end

    # Double-clic sur un bloc (éditeur de code)
    clicked_block = @blocks.reverse.find { |block| block.contains?(x, y) }
    if clicked_block
      @drag_start = nil
      @drag_offset = nil
  ...
  def motion_notify(widget, event)
    return true unless @drag_start || @temp_connection || @drag_port
    return true if @double_click_in_progress

    if @drag_port
      # Déplacement d'un port canvas
      @drag_port.x = event.x - @port_drag_offset[:x]
      @drag_port.y = event.y - @port_drag_offset[:y]
      queue_draw
      return true
    end
...
window.set_title("Éditeur de Blocs - Ports Libres dans le Canvas")
...
label_text = <<~TEXT
  Ports blocs: Ctrl+I/O | Ports libres: Ctrl+Maj+I/O | Connexions: Clic sortie → entrée
  Drag ports libres | Double-clic port: éditer | Clic-droit: supprimer
TEXT
