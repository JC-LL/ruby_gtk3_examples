  ...
  def on_canvas_click(event)
    ...
    when 1 # Clic gauche
      ...
      if x >= element[:x] && x <= element[:x] + element[:width] &&
        ...
        if delta_temps < 0.5 && delta_x < 10 && delta_y < 10 && @en_attente_double_clic
          # Double-clic détecté - ouvrir la fenêtre de saisie
          fenetre_saisie = SaisieFenetre.new("Saisie depuis double-clic")
          fenetre_saisie.show_all
          @en_attente_double_clic = false
        else
          # Premier clic - marquer la sélection et attendre un éventuel double-clic
          @element_selectionne[:selected] = true
          @canvas.queue_draw
          @en_attente_double_clic = true

          # Programmer la réinitialisation de l'attente de double-clic
